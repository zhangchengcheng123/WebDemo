...
const courseTong=[
    new CourseTong(1,"Github 开源之旅视频课程第一季：启程","assets/image/git.png",
    ...
    12,3736),
    new CourseTong(2,"css3基础","assets/image/css3.png",
    "CSS3在以前的基础上添加了诸多新的选择器以及新的样式。本课程将讲解CSS3新样式以及新特性.",
    12,2316),
    new CourseTong(3,"HTML5基础","assets/image/html.png",
    "HTML5是最新的一项Web标准，在原有HTML4的基础上定义了一些新的标签和新的JavaScript API。是原有HTML4标准的一个超集。本课程主要介绍HTML5的新特性以及相应API。",
    12,1311),
    new CourseTong(4,"Selenium IDE WEB自动化测试入门视频课程（中）","assets/image/text.png",
    ...
    12,3794),
    new CourseTong(5,"扩展的ICONIX软件过程实践","assets/image/iconix.png",
...
app.get('/api/courseTong',(req,res)=>{
    res.end(JSON.stringify(courseTong));
});
app.get('/api/courseTong/:id',(req,res)=>{
    res.json(courseTong.filter((ele)=>ele.id==req.params.id));
});
app.post('/api',(req,res)=>{
    res.json(courseTong);
})
...
app.get('/api/community/:id',(req,res)=>{
    res.json(community.filter((ele)=>ele.id==req.params.id));
});
app.post('/api',(req,res)=>{
    res.json(community);
})

app.get('/api/tongzhiTail',(req,res)=>{
    res.end(JSON.stringify(tongzhiTail));
});
app.get('/api/tongzhiTail/:id',(req,res)=>{
    res.json(tongzhiTail.filter((ele)=>ele.id==req.params.id));
});
app.post('/api',(req,res)=>{
